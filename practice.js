...
function divider(numbersArray){
  ...
  finalArr.push(evensArr);
  finalArr.push(oddsArr);
  return finalArr;
}
...
console.log(finder([0,3,4,5,6,7,9,14,17,24,25,26,29,30], getRandomArbitrary));
...
function addTen(numbers){
  ...
  for (var i = 0; i < numbers.length; i++){
   anotherArr.push(numbers[i] * 1 + 10);
...
function findHimself(){
  for (var i = 0; i < devMountainEmployees.length; i++){
    if (devMountainEmployees[i]=== devMountainEmployees[1]){
